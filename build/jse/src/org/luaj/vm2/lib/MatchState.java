...
class MatchState {
	...
	MatchState(LuaString s, int src_len, LuaString pattern, int pat_len, boolean push_idxs) {
		this.s = s;
		this.p = pattern;
	...
	private void add_s(Buffer lbuf, LuaString news, int str_off, int str_end) {
		int l = news.length();
		for (int i = 0; i < l; i++) {
			...
			if (b != StringLib.L_ESC) {
				...
				if (!Character.isDigit((char)b)) {
					...
					lbuf.append(s.substring(str_off, str_end));
				} else {
					lbuf.append(push_onecapture(b - '1', str_off, str_end).strvalue());
				}
			}
		}
	}
	...
	public void add_value(Buffer lbuf, int src_pos, int str_end, LuaValue repl) {
		switch (repl.type()) {
			...
			case LuaValue.TTABLE:
				...
				repl = repl.get(push_onecapture(0, src_pos, str_end));
				break;
		...
		if (!repl.toboolean()) { // nil or false? 
			repl = s.substring(src_pos, str_end); // keep original text
	...
	private LuaValue push_onecapture(int i, int src_pos, int end) {
		if (i >= this.level) {
			if (i == 0) {
				return s.substring(src_pos, end);
			} else {
	...
	private int[] capture_ints;
	private int capture_index_cur = 0;
	private LuaValue[] capture_vals;
	private int capture_vals_idx = 0;
	
	public int[] Capture_ints() {return capture_ints;}
	public LuaValue[] Capture_vals() {return capture_vals;}
	
	private void push_onecapture2(int i, int src_pos, int end) {
		if (i >= this.level) {
			if (i == 0) {
				if (push_idxs) {
					capture_ints[capture_index_cur++] = src_pos; 
					capture_ints[capture_index_cur++] = end; 
				}
				else {
					capture_vals[i] = s.substring(src_pos, end);
				}
			} else {
				throw new LuaError("invalid capture index");
			}
		} else {
			int len = capture_lens[i];
			if (len == CAP_UNFINISHED) {
				throw new LuaError("unfinished capture");
			}
			if (len == CAP_POSITION) {
				if (push_idxs) {
					int bgn = capture_bgns[i]; // +1?
					capture_ints[capture_index_cur++] = bgn; 
					capture_ints[capture_index_cur++] = bgn + (len == CAP_POSITION ? 0 : len); 
				}
				else {
					capture_vals[i] = LuaValue.valueOf(capture_bgns[i] + 1);					
				}
			} else {
				int bgn = capture_bgns[i];
				if (push_idxs) {
					capture_ints[capture_index_cur++] = bgn; 
					capture_ints[capture_index_cur++] = bgn + len; 					
				}
				else {
					capture_vals[i] = s.substring(bgn, bgn + len);
				}
			}
		}
	}
	public MatchState push_captures2(boolean wholeMatch, int src_pos, int end) {
		int nlevels = (this.level == 0 && wholeMatch) ? 1 : this.level;		
		if (push_idxs) {
			capture_ints = new int[nlevels * 2];
		}
		else {
			capture_vals = new LuaValue[nlevels];
		}
		if (nlevels == 0) {
			if (push_idxs) {
//				capture_ints[capture_vals_idx++] = -1;
			}
			else {
				capture_vals[capture_vals_idx++] = LuaValue.NONE;
			}
			return this;
		}
		for (int i = 0; i < nlevels; ++i)
			push_onecapture2(i, src_pos, end);
		return this;
//		return LuaValue.varargsOf(v);
	...
	public Varargs push_captures(boolean wholeMatch, int src_pos, int end) {
		...
		switch (nlevels) {
			...
			case 1: return push_onecapture(0, src_pos, end);
		}
		LuaValue[] v = new LuaValue[nlevels];
		for (int i = 0; i < nlevels; ++i)
			v[i] = push_onecapture(i, src_pos, end);
		return LuaValue.varargsOf(v);
	}

	private int check_capture(int l) {
		l -= '1'; // NOTE: don't know why this is a char ('1' => 49?)
	...
	private int classend(int pat_pos) {
		switch (p.luaByte(pat_pos++)) {
			case StringLib.L_ESC:
				...
				return pat_pos + 1;	
			case '[':
				if (p.luaByte(pat_pos) == '^')
					pat_pos++;
				do {
					if (pat_pos == pat_len) {
						LuaValue.error("malformed pattern (missing])");
					}
					if (p.luaByte(pat_pos++) == StringLib.L_ESC && pat_pos != pat_len)
						pat_pos++;
				} while (p.luaByte(pat_pos) != ']');
				return pat_pos + 1;
			default:
				return pat_pos;
		}
	...
	private boolean match_class(int cur, int cls) {
		...
		int char_data = cur == -1 ? 0 : CHAR_TABLE[cur];	// XOWA: handle cur == -1 else bounds error; note that match passes in -1 deliberately: "int previous = (soffset == 0) ? -1..."; DATE:2014-08-14 
	...
	private boolean matchbracketclass(int cur, int pat_pos, int ep) {
		boolean sig = true;
		if (p.luaByte(pat_pos + 1) == '^') {
			sig = false;
			pat_pos++;
		}
		while (++pat_pos < ep) {
			if (p.luaByte(pat_pos) == StringLib.L_ESC) {
				pat_pos++;
				if (match_class(cur, p.luaByte(pat_pos)))
					return sig;
			}
			else if ((p.luaByte(pat_pos + 1) == '-') && (pat_pos + 2 < ep)) {
				pat_pos += 2;
				if (p.luaByte(pat_pos - 2) <= cur && cur <= p.luaByte(pat_pos))
					return sig;
			}
			else if (p.luaByte(pat_pos) == cur) return sig;
		}
		return !sig;
	}
	...
	private boolean singlematch(int cur, int pat_pos, int ep) {
		switch (p.luaByte(pat_pos)) {
			case '.': return true;
			case StringLib.L_ESC: return match_class(cur, p.luaByte(pat_pos + 1));
			case '[': return matchbracketclass(cur, pat_pos, ep - 1);
			default: return p.luaByte(pat_pos) == cur;
		}
	}
	...
	private int matchbalance(int src_pos, int pat_pos) {
		...
		if (src_pos >= s.m_length) return NULL;	// XOWA: check bounds; EX:string_match('a', '^(.) ?%b()'); DATE:2014-08-13
		if (s.luaByte(src_pos) != p.luaByte(pat_pos))
			return NULL;
		else {
			int balance_bgn = p.luaByte(pat_pos);
			int balance_end = p.luaByte(pat_pos + 1);
			int balance_count = 1;
			while (++src_pos < src_len) {
				if (s.luaByte(src_pos) == balance_end) {
					if (--balance_count == 0)
						return src_pos + 1;
				}
				else if (s.luaByte(src_pos) == balance_bgn)
					balance_count++;
			}
		}
		return NULL;
	...
	private int max_expand(int src_pos, int pat_pos, int ep) {
		...
		while	(   src_pos + i < src_len
				&&	singlematch(s.luaByte(src_pos + i), pat_pos, ep))
			i++;
	...
	private int min_expand(int src_pos, int pat_pos, int ep) {
		int src_len = s.m_length;	// XOWA: cache string length; DATE: 2014-08-13
		for (;;) {
			int res = match(src_pos, ep + 1);
			if (res != NULL)
				return res;
			else if (src_pos < src_len && singlematch(s.luaByte(src_pos), pat_pos, ep))
	...
	private int match_capture(int src_pos, int l) {
		...
		if 	((src_len - src_pos) >= len
			&& LuaString.equals(s, capture_bgns[l], s, src_pos, len))
			return src_pos + len;
		else
			return NULL;
	}
	...
	public int match(int src_pos, int pat_pos) {
		while (true) {
			...
			switch (p.luaByte(pat_pos)) {
				case '(': // start capture
					if (++pat_pos < pat_len && p.luaByte(pat_pos) == ')') // position capture?
				...
				case StringLib.L_ESC:
					...
					switch (p.luaByte(pat_pos + 1)) {
						...
						case 'f': {// frontier?
							...
							int previous = (src_pos == 0) ? -1 : s.luaByte(src_pos - 1);
							if (							 matchbracketclass(previous				, pat_pos, ep - 1) ||
								 (src_pos < s.m_length && 	!matchbracketclass(s.luaByte(src_pos)	, pat_pos, ep - 1)))	// XOWA: (1) added bounds check of "src_pos < s.m_length"; DATE:2014-08-14; (2) fixed by changing from "matchbracketclass" to "!matchbracketclass"; PAGE:en.w:A; DATE:2016-01-28 
								return NULL;
							pat_pos = ep;
							continue;
						}
						default: {
							int c = p.luaByte(pat_pos + 1);
				...
				case '$':
					if (pat_pos + 1 == pat_len) // is the `$' the last char in pattern?
						return (src_pos == src_len) ? src_pos : NULL; // check end of string
			}
			
			int ep = classend(pat_pos);
			boolean m = src_pos < src_len && singlematch(s.luaByte(src_pos), pat_pos, ep);
			int pc = (ep < pat_len) ? p.luaByte(ep) : '\0';
			switch (pc) {
				case '?': // optional
					int res;
	...
	private static final byte[] CHAR_TABLE = makeCharTable();
